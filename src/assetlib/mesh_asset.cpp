...
void assets::unpack_mesh(MeshInfo *info, const char *sourceBuffer,
                         ...
                         char *indexBuffer) {
  // Decompression into temporal vector. TODO: streaming decompress directly on
  // the buffers
  ...
  // Copy index buffer
  memcpy(indexBuffer, decompressedBuffer.data() + info->vertexBufferSize,
         info->indexBufferSize);
}

auto assets::pack_mesh(MeshInfo *info, char *vertexData, char *indexData)
    -> assets::AssetFile {
  AssetFile file;
  file.type[0] = 'M';
  file.type[1] = 'E';
